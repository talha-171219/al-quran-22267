...
const IslamicBooks = () => {
  ...
  const handleBookClick = (book: Book) => {
    if (book.pdfUrl) {
      setSelectedPdf({ url: book.pdfUrl, title: book.titleBn });
    }
  };
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">
                  ইসলামী জ্ঞান অর্জনের জন্য গুরুত্বপূর্ণ বইগুলো এখানে পাবেন।
                  পিডিএফ ফরম্যাটে পড়ুন এবং জ্ঞান অর্জন করুন।
                </p>
              </div>
            </div>
          </Card>
          ...
          <div className="space-y-3">
            {books.length > 0 ? (
              books.map((book) => (
                <Card 
                  key={book.id} 
                  className={`p-4 transition-all border-l-4 border-l-primary/50 ${
                    book.pdfUrl 
                      ? 'hover:shadow-lg hover:scale-[1.02] cursor-pointer' 
                      : 'opacity-60'
                  }`}
                  onClick={() => book.pdfUrl && handleBookClick(book)}
                >
