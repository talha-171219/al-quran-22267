...
const MizanurRahmanAzhari = () => {
  ...
  useEffect(() => {
    fetch('/gojol/lectures_mizanur.json')
      ...
      .catch(err => {
        console.error('Failed to load Mizanur videos:', err);
        setVideos([]);
      });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <TopBar title="Mizanur Rahman Azhari" showBack />
      ...
      <main className="max-w-lg mx-auto px-4 py-6 space-y-6">
        <Card className="p-4">
          ...
          <h2 className="text-lg font-semibold mb-2">Mizanur Rahman Azhari</h2>
          <p className="text-sm text-muted-foreground mb-4">Lectures by Mizanur Rahman Azhari — tap to play</p>

          {videos.length === 0 ? (
            <div className="text-sm text-muted-foreground">No videos available yet. Provide a playlist JSON at <code>/gojol/lectures_mizanur.json</code> or give me the playlist link and I'll add the videos.</div>
          ) : (
...
export default MizanurRahmanAzhari;
